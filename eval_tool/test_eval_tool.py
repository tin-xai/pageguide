...
from eval_tool.step_confidence import compute_spec_confidence, g_grounding
...
class EvalToolTest(unittest.TestCase):
    ...
    def test_compute_spec_confidence_variants(self):
        # index grounded (1.0), loop 0.4, embedding progress 0.6 -> G_t = 0.8
    ...
    def test_compute_spec_confidence_progress_falls_back_to_neutral(self):
        step = {"action": "click", "target": {"text": "Buy"}, "loop": 0.0}  # text fallback grounding 0.7
        # G_t = 0.5*0.7 + 0.5*0.5 = 0.6
        self.assertAlmostEqual(compute_spec_confidence(step, "spec_full"), 0.6)
    ...
    def test_aggregate_task_modes(self):
        steps = [
            {"step": 1, "action": "click", "target": {"llmIndex": 1}, "loop": 0.0, "g_progress_score": 1.0},  # 1.0
            {"step": 2, "action": "click", "target": {"text": "x"}, "loop": 0.0, "g_progress_score": 0.0},     # 0.35
