...
PORT_PIDS="$(lsof -tiTCP:5000 -sTCP:LISTEN 2>/dev/null || true)"
if [ -n "$PORT_PIDS" ]; then
  echo "==> Stopping existing process(es) on port 5000: $PORT_PIDS"
  kill $PORT_PIDS 2>/dev/null || true
  sleep 1

  PORT_PIDS="$(lsof -tiTCP:5000 -sTCP:LISTEN 2>/dev/null || true)"
  if [ -n "$PORT_PIDS" ]; then
    echo "==> Force-stopping process(es) still on port 5000: $PORT_PIDS"
    kill -9 $PORT_PIDS 2>/dev/null || true
    sleep 1
  fi
fi

exec python eval_server/app.py
