...
def backfill_spec_progress(run_id: str, results: list[dict]) -> None:
    """Compute + persist the new-spec goal-relevance embeddings
    (`g_goal_relevance_score`) for any task missing them. Non-fatal: a missing key
    or network error leaves the step without the score (grounding-only fallback).
    """
...
def _recorded_action_steps(result: dict) -> set[int]:
    steps = set()
...
def missing_step_summary(results: list[dict]) -> dict:
    rows = []
    missing_any = 0
    missing_step_2 = 0
    for result in results:
        ...
        if steps:
            max_step = max(steps)
            missing = [n for n in range(1, max_step + 1) if n not in steps]
            if missing:
                missing_any += 1
            if 2 not in steps:
                missing_step_2 += 1
    ...
    return {
        ...
        "by_task": {row["task_id"]: row for row in rows if row.get("task_id")},
    }


def _format_missing_steps(missing: list[int]) -> str:
    if not missing:
        return ""
