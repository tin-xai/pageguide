...
def _resolve_profile() -> Iterator[str]:
    ...
    if persistent:
        ...
        with tempfile.TemporaryDirectory(prefix="pageguide-eval-") as tmp:
            yield tmp
...
class PlaywrightGuideRunner:
    ...
    async def run(self, tasks: list[EvalTask]) -> None:
        ...
        async with async_playwright() as pw:
            try:
                await self._run_in_browser(pw, tasks, headless=self.headless)
            except RuntimeError as exc:
                if self.headless and "Could not find PageGuide extension service worker" in str(exc):
                    self.patch_run(
                        self.run_id,
                        status="running",
                        warning="Headless Chromium did not expose the extension service worker; retrying headful.",
                    )
                    await self._run_in_browser(pw, tasks, headless=False)
                else:
                    raise

    async def _run_in_browser(self, pw: Any, tasks: list[EvalTask], headless: bool) -> None:
        ensure_chrome_profile_available()
        with _resolve_profile() as profile:
            self._check_stop()
            self.patch_run(self.run_id, current_phase="launching browser", task_total=len(tasks), progress=f"0/{len(tasks)}")
            ...
            RUN_CONTEXTS[self.run_id] = context
            try:
                self._check_stop()
                ...
                for index, task in enumerate(tasks, start=1):
                    ...
                    self.patch_run(
                        ...
                        current_task_name=task.task,
                        task_index=index,
                        task_total=total,
                        progress=f"{index}/{total}",
                        current_step=0,
                        current_phase="loading page",
                    )
                    result = await self._run_task(context, extension_page, task, index, total)
                    self._check_stop()
                    save_task_result(self.run_id, task.task_id, result)
                    self.patch_run(self.run_id, status="running", completed_tasks=index, task_total=total)
            finally:
                RUN_CONTEXTS.pop(self.run_id, None)
                await context.close()
